...
TargetEvent TargetExtractor::targetEventFromTarget(const Target *_target) {
    ...
    case TargetType::Peak :
    {
        ...
        return TargetEvent::createTargetEventWithTime(_target->getActiveMoietyId(),
                                       ...
                                       Unit("ug/l"),
                                       _target->m_unit,
                                       UnitManager::translateToUnit(_target->m_valueMin, _target->m_unit, Unit("ug/l")),
                                       UnitManager::translateToUnit(_target->m_valueBest, _target->m_unit, Unit("ug/l")),
                                       UnitManager::translateToUnit(_target->m_valueMax, _target->m_unit, Unit("ug/l")),
                                       _target->m_tMin,
                                       _target->m_tBest,
                                       _target->m_tMax);
    } break;
    ...
    case TargetType::Auc24OverMic :
    {
        return TargetEvent::createTargetEventWithMicAndTime(_target->getActiveMoietyId(),
                                       ...
                                       Unit("ug*h/l"),
                                       _target->m_unit,
                                       UnitManager::translateToUnit(_target->m_valueMin, _target->m_unit, Unit("ug*h/l")),
                                       UnitManager::translateToUnit(_target->m_valueBest, _target->m_unit, Unit("ug*h/l")),
                                       UnitManager::translateToUnit(_target->m_valueMax, _target->m_unit, Unit("ug*h/l")),
                                       UnitManager::translateToUnit(_target->m_mic, _target->m_micUnit, Unit("ug*h/l")),
                                       _target->m_micUnit,
                                       _target->m_tMin,
                                       _target->m_tBest,
                                       _target->m_tMax);
...

    {
        ...
        return TargetEvent::createTargetEventWithMicAndTime(_target->getActiveMoietyId(),
                                       ...
                                       Unit(""),
                                       _target->m_unit,
                                       UnitManager::translateToUnit(_target->m_valueMin, _target->m_unit, Unit("")),
                                       UnitManager::translateToUnit(_target->m_valueBest, _target->m_unit, Unit("")),
                                       UnitManager::translateToUnit(_target->m_valueMax, _target->m_unit, Unit("")),
                                       UnitManager::translateToUnit(_target->m_mic, _target->m_micUnit, Unit("ug/l")),
                                       _target->m_micUnit,
                                       _target->m_tMin,
                                       _target->m_tBest,
                                       _target->m_tMax);
    ...
    case TargetType::Auc24DividedByMic :
    {
        ...
        return TargetEvent::createTargetEventWithMicAndTime(_target->getActiveMoietyId(),
                                       ...
                                       Unit("h"),
                                       _target->m_unit,
                                       UnitManager::translateToUnit(_target->m_valueMin, _target->m_unit, Unit("h")),
                                       UnitManager::translateToUnit(_target->m_valueBest, _target->m_unit, Unit("h")),
                                       UnitManager::translateToUnit(_target->m_valueMax, _target->m_unit, Unit("h")),
                                       UnitManager::translateToUnit(_target->m_mic, _target->m_micUnit, Unit("ug/l")),
                                       _target->m_micUnit,
                                       _target->m_tMin,
                                       _target->m_tBest,
...
TargetEvent TargetExtractor::targetEventFromTargetDefinition(const TargetDefinition *_target) {
    ...
    case TargetType::Residual :
        ...
        return TargetEvent::createTargetEventWithoutTimeAndMic(_target->getActiveMoietyId(),
                                       ...
                                       Unit("ug/l"),
                                       _target->getUnit(),
                                       UnitManager::translateToUnit(_target->getCMin().getValue(), _target->getUnit(), Unit("ug/l")),
                                       UnitManager::translateToUnit(_target->getCBest().getValue(), _target->getUnit(), Unit("ug/l")),
                                       UnitManager::translateToUnit(_target->getCMax().getValue(), _target->getUnit(), Unit("ug/l")));
    ...
    case TargetType::CumulativeAuc :
        ...
        return TargetEvent::createTargetEventWithTime(
                    ...
                    Unit("ug*h/l"),
                    _target->getUnit(),
                    UnitManager::translateToUnit(_target->getCMin().getValue(), _target->getUnit(), Unit("ug*h/l")),
                    UnitManager::translateToUnit(_target->getCBest().getValue(), _target->getUnit(), Unit("ug*h/l")),
                    UnitManager::translateToUnit(_target->getCMax().getValue(), _target->getUnit(), Unit("ug*h/l")),
                    Tucuxi::Common::Duration(
                        std::chrono::minutes(static_cast<int>(_target->getTMin().getValue()))),
                    Tucuxi::Common::Duration(
                        std::chrono::minutes(static_cast<int>(_target->getTBest().getValue()))),
    ...
    case TargetType::Auc24OverMic :
        ...
        return TargetEvent::createTargetEventWithMicAndTime(
                    ...
                    UnitManager::translateToUnit(_target->getCBest().getValue(), _target->getUnit(), Unit("ug*h/l")),
                    UnitManager::translateToUnit(_target->getCMax().getValue(), _target->getUnit(), Unit("ug*h/l")),
                    UnitManager::translateToUnit(_target->getMic().getValue(), _target->getMicUnit(), Unit("ug/l")),
                    Unit("ug/l"),
                    Tucuxi::Common::Duration(
                        std::chrono::minutes(static_cast<int>(_target->getTMin().getValue()))),
                    Tucuxi::Common::Duration(
    ...
    case TargetType::TimeOverMic :
        ...
        return TargetEvent::createTargetEventWithMicAndTime(
                    ...
                    Unit("h"),
                    _target->getUnit(),
                    UnitManager::translateToUnit(_target->getCMin().getValue(), _target->getUnit(), Unit("h")),
                    UnitManager::translateToUnit(_target->getCBest().getValue(), _target->getUnit(), Unit("h")),
                    UnitManager::translateToUnit(_target->getCMax().getValue(), _target->getUnit(), Unit("h")),
                    UnitManager::translateToUnit(_target->getMic().getValue(), _target->getMicUnit(), Unit("ug/l")),
                    Unit("ug/l"),
                    Tucuxi::Common::Duration(
                        std::chrono::minutes(static_cast<int>(_target->getTMin().getValue()))),
    ...
    case TargetType::PeakDividedByMic :
        ...
        return TargetEvent::createTargetEventWithMicAndTime(
                    ...
                    Unit(""),
                    _target->getUnit(),
                    UnitManager::translateToUnit(_target->getCMin().getValue(), _target->getUnit(), Unit("")),
                    UnitManager::translateToUnit(_target->getCBest().getValue(), _target->getUnit(), Unit("")),
                    UnitManager::translateToUnit(_target->getCMax().getValue(), _target->getUnit(), Unit("")),
                    UnitManager::translateToUnit(_target->getMic().getValue(), _target->getMicUnit(), Unit("ug/l")),
                    Unit("ug/l"),
                    Tucuxi::Common::Duration(
                        std::chrono::minutes(static_cast<int>(_target->getTMin().getValue()))),
