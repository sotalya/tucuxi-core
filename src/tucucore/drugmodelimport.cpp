...
{
    if (unit.toString() == "d") {
        return Duration(std::chrono::seconds(static_cast<long>(duration * 3600.0 * 24.0)));
    }
    if (unit.toString() == "min") {
        return Duration(std::chrono::seconds(static_cast<long>(duration * 60.0)));
    }
    if (unit.toString() == "m") {
        return Duration(std::chrono::seconds(static_cast<long>(duration * 60.0)));
    }
    if (unit.toString() == "h") {
        return Duration(std::chrono::seconds(static_cast<long>(duration * 3600.0)));
    }
    if (unit.toString() == "s") {
        return Duration(std::chrono::seconds(static_cast<long>(duration)));
    }
    return Duration();
}
...
TargetDefinition* DrugModelImport::extractTarget(Tucuxi::Common::XmlNodeIterator _node)
{
    ...
    target = new TargetDefinition(type, unit, "",
                                  ...
                                  std::make_unique<SubTargetDefinition>("tMin", translateToUnit(tMin->getValue(), tUnit, Unit("m")), tMin->getOperation()),
