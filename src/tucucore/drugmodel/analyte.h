...
//    std::string getName() const { return m_name;}
    ...
    void setAnalyteId(std::string _analyteId) { m_analyteId = AnalyteId(std::move(_analyteId));}
    ...
    INVARIANTS(
            INVARIANT(Invariants::INV_ANALYTE_0001, (!m_analyteId.empty()), "An analyte does not have an Id")
            ...
            INVARIANT(Invariants::INV_ANALYTE_0003, (m_residualErrorModel->checkInvariants()), Tucuxi::Common::Utils::strFormat("The analyte %s has an error in the error model", m_analyteId.toString().c_str()))
            INVARIANT(Invariants::INV_ANALYTE_0004, (!m_unit.isEmpty()), Tucuxi::Common::Utils::strFormat("The analyte %s has no unit", m_analyteId.toString().c_str()))
            )
...
public:
    ...
    void setId(AnalyteGroupId _id) { m_analyteSetId = std::move(_id);}
    ...
    void setPkModelId(std::string _pkModelId) { m_pkModelId = std::move(_pkModelId);}
    std::string getPkModelId() const { return m_pkModelId;}

    void setDoseUnit(TucuUnit _unit) { m_doseUnit = std::move(_unit);}
    ...
    INVARIANTS(
            ...
            INVARIANT(Invariants::INV_ANALYTESET_0002, (!m_pkModelId.empty()), Tucuxi::Common::Utils::strFormat("The analyte group %s has no PK model Id", m_analyteSetId.toString().c_str()))
            INVARIANT(Invariants::INV_ANALYTESET_0003, (!m_analytes.empty()), Tucuxi::Common::Utils::strFormat("The analyte group %s has no analytes", m_analyteSetId.toString().c_str()))
            LAMBDA_INVARIANT(Invariants::INV_ANALYTESET_0004, {bool ok = true;for(size_t i = 0; i < m_analytes.size(); i++) {ok &= m_analytes[i]->checkInvariants();} return ok;}, Tucuxi::Common::Utils::strFormat("There is an error in an analyte of the analyte group %s", m_analyteSetId.toString().c_str()))
            INVARIANT(Invariants::INV_ANALYTESET_0005, (m_dispositionParameters != nullptr), Tucuxi::Common::Utils::strFormat("The analyte group %s has no disposition parameters", m_analyteSetId.toString().c_str()))
            INVARIANT(Invariants::INV_ANALYTESET_0006, (m_dispositionParameters->getNbParameters() > 0), Tucuxi::Common::Utils::strFormat("The analyte group %s has no disposition parameters", m_analyteSetId.toString().c_str()))
            INVARIANT(Invariants::INV_ANALYTESET_0007, (m_dispositionParameters->checkInvariants()), Tucuxi::Common::Utils::strFormat("The analyte group %s has an error in its disposition parameters", m_analyteSetId.toString().c_str()))
            )
