...
public:
    ...
    INVARIANTS(
            ...
            INVARIANT(Invariants::INV_ANALYTESET_0002, (m_pkModelId.size() > 0), Tucuxi::Common::Utils::strFormat("The analyte group %s has no PK model Id", m_analyteSetId.toString().c_str()))
            INVARIANT(Invariants::INV_ANALYTESET_0003, (m_analytes.size() > 0), Tucuxi::Common::Utils::strFormat("The analyte group %s has no analytes", m_analyteSetId.toString().c_str()))
            LAMBDA_INVARIANT(Invariants::INV_ANALYTESET_0004, {bool ok = true;for(size_t i = 0; i < m_analytes.size(); i++) {ok &= m_analytes.at(i)->checkInvariants();} return ok;}, Tucuxi::Common::Utils::strFormat("There is an error in an analyte of the analyte group %s", m_analyteSetId.toString().c_str()))
