...
public:
    ...
    INVARIANTS(
            ...
            INVARIANT(Invariants::INV_ACTIVEMOIETY_0002, (!m_analyteIds.empty()), Tucuxi::Common::Utils::strFormat("The active moiety %s has no analyte Ids", m_id.toString().c_str()))
            LAMBDA_INVARIANT(Invariants::INV_ACTIVEMOIETY_0003, {bool ok = true;for(size_t i = 0; i < m_analyteIds.size(); i++) {ok &= !m_analyteIds[i].empty();} return ok;}, Tucuxi::Common::Utils::strFormat("There is an empty analyte Id in the active moiety %s", id.c_str()))
