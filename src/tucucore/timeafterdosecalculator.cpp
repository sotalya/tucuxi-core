        ...
        const Samples& _samples, const DosageHistory& _dosageHistory) const
{
    ...
    for (auto const& tr : trs) {
        ...
        if (typeid(*dosage) == typeid(SimpleDoseList)) {
            SimpleDoseList const* const dosagePtr = static_cast<SimpleDoseList const* const>(dosage);

            if (startDate > dosagePtr->getFirstDosageDate()) {
                logHelper.error(
                        std::string("Time range start date is ") + startDate.str()
            ...
            if (endDate < dosagePtr->getLastDosageDate()) {
                logHelper.error(
                        std::string("Time range end date is ") + endDate.str()
                        + std::string(", but last dosage in the range is at ") + dosagePtr->getLastDosageDate().str());
                throw std::runtime_error("Time range end date error");
            }
        }
        if (typeid(*dosage) == typeid(SingleDoseAtTimeList)) {
            SingleDoseAtTimeList const* const dosagePtr = static_cast<SingleDoseAtTimeList const* const>(dosage);
