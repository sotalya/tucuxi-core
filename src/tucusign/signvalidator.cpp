...
SignatureError SignValidator::verifySignature(
...
SignatureError SignValidator::verifyChain(Botan::X509_Certificate& userCert, Botan::X509_Certificate& signingCert)
{
    ...
    std::string rootCertPem =
            std::string((std::istreambuf_iterator<char>(ROOTCAFile)), std::istreambuf_iterator<char>());
...
SignatureError SignValidator::validateSignature(Signature& signature)
{
    ...
    try {
        ...
        // verify signature
        SignatureError isSignatureValid =
                verifySignature(userPublicKey, signature.getValue(), signature.getSignedData());
