...
cmd_coverage() {
  ...
  local MERGED_INFO="$COVERAGE_ROOT/lcov/coverage.info"
...
EOF
  ...
  for MODULE in "${MODULES[@]}"; do
    ...
    # Find the test binary (named tucutest<module>)
    if [[ ! -x "$MOD_BIN" ]]; then
      die "Test binary not found: $MOD_BIN"
    fi
    ...
    set -e
    if [[ $RC -ne 0 ]]; then
      echo "==> WARN: $MODULE tests returned non-zero ($RC). Generating coverage anyway."
    fi
  ...
  rm -rf "$HTML_DIR"
  genhtml "$MERGED_INFO" --output-directory "$HTML_DIR" >/dev/null
