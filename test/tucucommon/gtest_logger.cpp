...
{
public:
    void reset(const char* _fileName, std::string m_path)
    {
...
TEST (Common_LoggerTest, basic){
    ...
    gTestLogger.checkLogfileLine(infile, "debug", "Tcho les topiots");
    gTestLogger.checkLogfileLine(infile, "info", "Tcho les topiots");
    gTestLogger.checkLogfileLine(infile, "warning", "2 + 2 = 4");
    gTestLogger.checkLogfileLine(infile, "error", "Tcho les topiots");
...
TEST (Common_LoggerTest, crashes){
    ...
    gTestLogger.reset("LogTest1.log", appFolder);
